...
class TriangleTest < Minitest::Test
  ...
  def test_no_size_is_illegal
    assert_raises(ArgumentError) { triangle = Triangle.new(0, 0, 0) }
  end

  def test_negative_size_is_illegal
    assert_raises(ArgumentError) { triangle = Triangle.new(3, 4, -5) }
  end

  def test_size_inequality_is_illegal
    assert_raises(ArgumentError) { triangle = Triangle.new(1, 1, 3) }
  end

  def test_size_inequality_is_illegal_2
    assert_raises(ArgumentError) { triangle = Triangle.new(7, 3, 2) }
  end

  def test_size_inequality_is_illegal_3
    assert_raises(ArgumentError) { triangle = Triangle.new(10, 1, 3) }
  end

  def test_size_inequality_is_illegal_4
    assert_raises(ArgumentError) { triangle = Triangle.new(1, 1, 2) }
  end
end
